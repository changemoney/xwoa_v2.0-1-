...
<?php if(!empty($plugin["calendar"])): ?><link href="/Public/Ins/css/plugins/fullcalendar/fullcalendar.css" rel="stylesheet">
	...
	<?php echo W('PageHeader/simple',array('编辑：'.$flow_type['name']));?>
	...
	<form method='post' id="form_data" name="form_data" enctype="multipart/form-data" class="well form-horizontal">
		...
		<div class="form-group col-sm-6">
			...
			<div class="col-sm-8">
				<p class="form-control-static">
					{vo.doc_no}
					<!-- <?php echo ($flow_type["doc_no_format"]); ?>   2016.1.28 origin 修改编辑审批单号查看问题  -->
				</p>
			</div>
		</div>
		...
		<div class="form-group col-sm-6">
			...
			<div class="col-sm-8">
				<p class="form-control-static">
					<?php echo (to_date($vo["create_time"],'Y-m-d')); ?>
				</p>
			</div>
		</div>
	...
	<script type="text/javascript">
		...
		function del() {
			ui_confirm('确定要删除吗?', function() {
				sendAjax("<?php echo U('del');?>", 'id=<?php echo ($vo["id"]); ?>', function(data) {
					if (data.status) {
