...
<?php if(!empty($plugin["calendar"])): ?><link href="/Public/Ins/css/plugins/fullcalendar/fullcalendar.css" rel="stylesheet">
	...
	<?php echo W('PageHeader/simple',array('name'=>$folder_name));?>
	...
	<div class="row">
		...
		<div class="col-sm-9">
			<form id="form_data" name="form_data" method="post" class="well form-horizontal clearfix">
				...
				<?php if($has_pid): ?><div class="form-group col-xs-12">
...
<script type="text/javascript">
	...
	<script type="text/javascript">
		function add() {
			// $("#opmode").val("add");
			// $("#id").val("");
			// sendForm("form_data", "", "/index.php?m=&c=Mail&a=folder_manage");
	   winopen("<?php echo U('addfolder');?>", 560, 470);
		};
		...
		function select_pid() {
			if ($("div.sub_left_menu a.active").length > 0) {
				winopen("<?php echo U('UserFolder/winpop');?>", 560, 470);
		...
		$(document).ready(function() {
			$(".sub_left_menu .tree_menu  a").click(function() {
				...
				sendAjax("<?php echo U('UserFolder/read');?>", "id=" + $(this).attr("node"), function(data) {
					showdata(data);
				});
				return false;
			});
