	...
	<script type="text/javascript">
		...
		function save() {
			...
			$("#cc.selected li").each(function() {
				emp_no = $(this).attr('data');
				console.log('弹出框显示页面emp_no数据：',emp_no);
				name = jQuery.trim($(this).text());
				console.log('弹出框显示页面name数据：',name);
				name = name.replace(/<.*>/, '');
				html_string = conv_inputbox_item(name, emp_no);
				console.log('弹出框显示页面html数据：',html_string);
		...
		function showdata(result) {
			//console.log('弹出框showdata数据：',result);
			console.log('弹出框showdata数据：',result);
			$("#addr_list").html("");
			if ( type = $("input[name='type']:checked").val() == "dept") {
				...
				console.log('弹出框html_string数据：',html_string);
