...
class Model {
     ...
     */
     protected function _facade($data) {
         \Think\Log::write('--- _facade 111111 2222----','DEBUG');
        // 检查数据字段合法性
        if(!empty($this->fields)) {
            ...
            foreach ($data as $key=>$val){
                ...
                if(!in_array($key,$fields,true)){
                    ...
                    unset($data[$key]);
                    \Think\Log::write('--- 关键值 key ----'.$key,'DEBUG');
    ...
    protected function _before_write(&$data) {
        ...
        if(!empty($data)){
            $pwd = $data['mail_pwd'];
            \Think\Log::write('--- _before_write $pwd---'.$pwd,'DEBUG');

//             $json = json_encode($pwd);
//             // 转为base64字符串
...
//                  }
//             }
             //$data['mail_pwd'] = "123456";//$sign;
//             \Think\Log::write('--- _before_write 加密后的$pwd---'.$data['mail_pwd'],'DEBUG');
        }
    }
    ...
    public function add($data='',$options=array(),$replace=false) {
        ...
        if(false !== $result && is_numeric($result)) {
            ...
            if($insertId) {
                ...
                \Think\Log::write('--- 流程插入的数据库ID ----'.$insertId,'DEBUG');
                return $insertId;
            }
            if(false === $this->_after_insert($data,$options)) {
                return false;
