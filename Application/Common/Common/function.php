...
// 	    \Think\Log::write('--- list to tree $key ----'.$key,'DEBUG');
// 	    \Think\Log::write('--- list to tree $tree[$key] ----'.$tree[$key]['name'],'DEBUG');
// 	}
...
function left_menu($tree, $level = 0) {
    \Think\Log::write('---left_menu----','DEBUG');
	$level++;
	$html = "";
	if (is_array($tree)) {
		...
		foreach ($tree as $val) {
			if (isset($val["name"])) {
				$title = $val["name"];
				\Think\Log::write('---left_menu $title ----'.$title,'DEBUG');
				if (!empty($val["url"])) {
					$url = U($val['url']);
				} else {
					$url = "#";
...
function show_contact($str, $mode = "show") {
	...
	if (!empty($str)) {
		...
		if (count($contacts) > 1) {
			...
			$tmp = $tmp . "<span data=\"alex\"><nobr><b  title=\"alex - 组长\">alex</b><a class=\"del\" title=\"删除\"><i class=\"fa fa-times\"></i></a></nobr></span>";
		} else {
...
//费用获取
function get_payment_is_read($id) {
    $where['flow_id'] = array('eq', $id);
