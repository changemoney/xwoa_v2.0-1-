...
class  FlowModel extends CommonModel {
	...
	function _conv_email_list($val){
	    ...
	    foreach ($arr_auditor as $auditor) {
	        //if (strpos($auditor, "emp") !== false) {
	            ...
	            $model = M("User");
	            $where = array();
	            $where['emp_no'] = array('eq', $emp);;
	            $emp_list = $model -> where($where) -> select();
	            $emp_list = rotate($emp_list);
	            if (!empty($emp_list)) {
	                $str_email_account .= implode(",", $emp_list['email']) . "|";
	            }
	        //}
	    }
	    
	    return $str_email_account;
	}

	function _conv_email_content($val){
	...
	function is_last_confirm($flow_id) {
		...
		if (strpos($emp, get_emp_no()) !== false) {
			return true;
		}
