...
class FlowController extends HomeController {
	...
	function payment($id) {
	    ...
	    $Payment = M("Payment");
	    $where['emp_no'] = get_emp_no();;
	    $log_list = $Payment -> where($where) -> select();
	    $map = $this -> _search();
	    //查询费用
	    $this -> _list($Payment, $map, 'create_time desc');
	    $this -> assign("payment", '个人申请费用汇总');
	    $this -> display();
	}
	...
	protected function _insert($name = CONTROLLER_NAME) {
	        ...
	        if($type == "62" || $type == "63" || $type == "64"){
	            $body = D("Flow") ->_conv_multi_email_content($content);
	        }else {
	        ...
	        if ($flow_id !== false) {//保存成功
	            ...
	            if(!empty($email_confirm)){
	                $this -> send_mail($email_confirm,$email_consult,$body,$flow_id);
	...
	public function approve() {
		...
		$flow_id = $model -> flow_id;//取出flow id
		$step = $model -> step;
		$where['flow_id'] = array('eq', $flow_id);
		$model -> where($where) -> setField('result', 1);
		$model -> where($where) -> setField('update_time', time());
		//保存当前数据对象
		$list = $model -> save();
        ...
        $comment = $model -> where($where) -> getField('comment');
		if ($list !== false) {//保存成功
	...
	private function send_mail($to,$cc,$body,$flow_id){
	    //引入邮件类，调用并发送邮件
	    import("@.Controller.MailController");
	    //从PHPMailer目录导入class.send.php类文件
	    $mail = new MailController(true);
	    // the true param means it will throw exceptions on errors, which we need to catch
	    $mail -> send_flow_mail($to,$cc,$body,$flow_id);
	    // telling the namespace Home\Controller;
	}
	
	//回复费用审批邮件
	private function send_reply_mail($flow_id){
	    //引入邮件类，调用并发送邮件
	    import("@.Controller.MailController");
	    //从PHPMailer目录导入class.send.php类文件
	    $mail = new MailController(true);
	...
	private function insert_multi_payment($flow_id){
	    ...
	    foreach ($arr_row as $item) {	        
	        ...
	        $temp[4] = $this -> filter_string($temp[4]);
	        $temp[5] = $this -> filter_string($temp[5]);
	        $temp[6] = $this -> filter_string($temp[6]);
	        $temp[7] = $this -> filter_string($temp[7]);
	        $temp[8] = $this -> filter_string($temp[8]);
	        $temp[9] = $this -> filter_string($temp[9]);

	        $data = array();
	        $data['user_id'] = $user_id;//申请人ID
	        $data['user_name'] = $user_name;//申请人名称
	        $data['group_id'] = $group_id;//模块
	        $data['create_time'] = time();//申请时间
	        $data['pay_object'] = $object;//费用标题
	        
	        $data['pay_type'] = $temp[1];
	        $data['pay_to'] = $temp[2];
	        $data['pay_account'] = $temp[3];
	        $data['pay_bank'] = $temp[4];
	        $data['pay_bankaddress'] = $temp[5];
	...
	private function filter_string($arr){
	    $arr = str_replace('"','',$arr);
	    $arr = str_replace('{','',$arr);
	    $arr = str_replace('}','',$arr);
	    $arr = str_replace('\r','<br/>',$arr);
	    $arr = str_replace('\n','',$arr);
	    $arr = str_replace('\\', '',$arr);
	    $val_arr = explode(",", $arr);
	    $val = $val_arr[0];
