...
class MailController extends HomeController {
	 ...
	 *
	 * @param 
	 * @param mail content $body
	 * @return
	 * 
	 */
	public function send_flow_mail($to,$cc,$body,$flow_id,$name){
	    //获取邮件账户函数
	    $mail_account = $this -> _get_mail_account();
	    //新建一个邮件发送类对象
	    import("@.ORG.Util.send");
	    ...
	    try{
	       ...
	       // }
	        ...
	        // 发送 HTML邮件
	        $mail->IsHTML (false );
	        $subject = D("Flow")-> where("id= $flow_id")-> getField('name');
	...
	public function reply_flow_mail($flow_id) {
	    ...
	    if(strlen($cc) > 1){
	        $cc_arr = "<p style='line-height:8px;'><b>&nbsp;抄送：</b>&nbsp;$cc</p>";
	    }else{
	        $cc_arr = "";
	    }
	    ...
	    $flow_comment = $flow_log -> where($where) -> getField("comment");
	    
	    if(strlen($flow_comment) > 1){
	        $reply_comment = $flow_comment;
	    }else{
...
// 							<p style='line-height:8px;padding-top:3px;'>
// 								<b>&nbsp;发件人：</b>&nbsp;$from
// 							</p>
...
// 						</div></div>".$body;
	    ...
	    try{
	       ...
	       // $mail -> AddReplyTo($mail_account['email'], $mail_account['mail_name']);
	        ...
	        $mail->IsHTML (false );
	        // 邮件主题
	        $mail->Subject = "回复: ".$subject;//"."=?UTF-8?B?" . base64_encode($decode_subject) . "?=";
	        // 邮件内容
	    ...
	    if (!empty($str)) {
	        ...
	        if (count($contacts) > 1) {
	            foreach ($contacts as $contact) {
	                ...
	                if ($mode == "edit") {
	                    $tmp = $tmp . "<span data=\"$data\"><nobr>  title=\"$name - $data\">$name<a class=\"del\" title=\"删除\"><i class=\"fa fa-times\"></i></a></nobr></span>";
	                } else {
